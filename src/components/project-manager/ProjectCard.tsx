...
import { Link } from "react-router"; // you said you use react-router imports
...
const ProjectCard: React.FC<Props> = ({ project, listView = false, onEdit, onDelete }) => {
  ...
  const linkTo = `/project/${encodeURIComponent(project.id)}`;

  return (
    <Link to={linkTo} className="group" aria-label={`Open project ${project.name}`} >
      <article
        className={
          "relative rounded-2xl overflow-hidden shadow-sm transform transition-all duration-300 " +
